--[[
    ...
    Sea 1/2/3 auto-detect. Real remote attacks. Highlight ESP. Sliders.
]]
...
local function do_attack(target)
    ...
    if RE.RegisterAttack then
        pcall(function() RE.RegisterAttack:FireServer(0) end)
    end
    if RE.RegisterHit then
        pcall(function() RE.RegisterHit:FireServer({[1] = tr}) end)
    end
    if RE.SwordHit then
        pcall(function() RE.SwordHit:FireServer({[1] = tr}) end)
    end
...
RunService.Heartbeat:Connect(function()
    ...
    if S.current_chest and S.current_chest.Parent then
        ...
        if d < 15 then
            ...
            pcall(function()
                r.CFrame = CFrame.new(S.current_chest.Position + Vector3.new(0, 3, 0))
            end)
        end
        return
    end
...
function module.build(hub, CFG, ICON)
    ...
    hub.toggle(farm, "Auto Fruit Sniper", "Teleports to fruit spawns", ICON.fruit,
    ...
    hub.toggle(farm, "Auto Grab Fruits", "Picks up dropped fruit nearby", ICON.fruit,
    ...
    hub.slider(move, "Walk Speed", "Drag to adjust", 16, 300, 16, ICON.run,
        function(v) S.speed = v end)
    hub.slider(move, "Jump Power", "Drag to adjust", 50, 250, 50, ICON.run,
    ...
    hub.button(move, "Reset Speed/Jump", "Back to defaults", ICON.run, function()
    ...
    local tp_page = hub.page("Teleport · Sea " .. tostring(SEA), ICON.tp)
    local islands = SEA_ISLANDS[SEA] or SEA_ISLANDS[1]
    for name, pos in pairs(islands) do
        hub.button(tp_page, name, "Teleport to " .. name, ICON.tp, function()
            local r = root(); if r then r.CFrame = CFrame.new(pos + Vector3.new(0, 8, 0)) end
        end)
    end
