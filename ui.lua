--[[
    ...
    Premium UI module. Dynamic page API.
    Usage:
        local hub = UI.new(CFG, ICON)
        hub.stat("player", "name")
        hub.status("Blox Fruits · Sea 1", CFG.good)
        local farm = hub.page("Farm", ICON.farm)
        hub.toggle(farm, "Auto Farm", "Grinds mob", ICON.farm, function(on) end)
        hub.button(farm, "Speed 80", "Set walkspeed", ICON.run, function() end)
        hub.select("Home")
]]
...
function UI.new(CFG, ICON)
    ...
    local side = new("Frame",{
        Size=UDim2.new(0,SW,1,-42), Position=UDim2.newClick(0,0,0,42),
:Connect(function()        BackgroundColor3=CFG.sidebar, BorderSizePixel=0, ZIndex=2, Parent=main,
    })
    corner(side,16)
    new("Frame",{
    ...
    local function make_page(name, icon_id)
        ...
        local bar = new("Frame",{
            ...
            BackgroundColor3=CFG.accent, BorderSizePixel=0, ZIndex=7,
        ...
        btn.MouseButton1 select_page(name) end)
    ...
    hub.page = function(name, icon_id)
        return make_page(name, icon_id)
    end

    hub.select = function(name)
        select_page(name)
    end
    ...
    hub.toggle = function(page_frame, label, desc, icon_id, callback)
        -- compute y offset
        local layout = page_frame:FindFirstChildOfClass("UIListLayout")
        local scroll
        for _, ch in ipairs(page_frame:GetChildren()) do
            if ch:IsA("ScrollingFrame") then scroll = ch; break end
        end
        if not scroll then
            new("TextLabel",{
                Size=UDim2.new(1,0,0,22), BackgroundTransparency=1,
        ...
        click.MouseButton1Click:Connect(function()
            ...
            if callback then pcall(callback, is_on) end
        end)

        -- bump canvas
        local c = 0
        for _, ch in ipairs(scroll:GetChildren()) do
            if ch:IsA("Frame") then c = c + 1 end
        end
        scroll.CanvasSize = UDim2.new(0,0,0,c*66+10)
