--[[
    ...
    Premium UI module with dynamic page API.
    Usage:
...
function UI.new(CFG, ICON)
    ...
    local dh = new("TextButton", {
        ...
        TextXAlignment = Enum.TextXAlignment.Left,
        Text = "   Discord   ▸", AutoButtonColor = false,
        ZIndex = 7, Parent = dd,
    })
    ...
    open_btn.MouseButton1Click:Connect(function()
        ...
        if request then
            pcall(function() request({ Url = CFG.discord, Method = "GET" }) end)
        end
    end)
    ...
    -- ensure_scroll helper for toggle/button
